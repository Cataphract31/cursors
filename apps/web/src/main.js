...
let AC=null, muted=false, masterVol=.7;
let snd=null;
...
function noiseBurst(dur,v,delay){
  ...
  try{
    ...
    const g=c.createGain(); g.gain.value=v*masterVol*waveF()*synthF();
...
$("#vf-slider").addEventListener("change",()=>{ if(!muted) sysSnd("balloon",.5); });
...
const store={
  ...
  load(){
    ...
    this.data.saver=this.data.saver||{t:"pipes",wait:3};
...
const AOT_Z=4900, FIX_Z=6000; let fixTop=0;
...
function fitWin(el){
  ...
  if(MOBILE&&!el.classList.contains("fixed")) return;
  const r=el.getBoundingClientRect();
...
function suspendApp(id){
  ...
  if(id==="win-mine") mine.pause();
  if(id==="win-hearts") hearts.pause();
  if(id==="win-paint") paint.commit();
  if(id==="win-tourxp") tourxp.stop();
  if(id==="win-sndrec"&&snd) snd.stopRecorder();
...
function closeWin(id,opts){
  try{ if(id==="win-magnifier"&&access.magnifying()) access.stopMagnifier(); }catch(e){}
  if(id==="win-copy") fileOpCancel=true;
  dlgOwner.delete(id);
...
function wireWindow(w){
  ...
  tb.addEventListener("pointerdown",e=>{
    if(e.target.closest(".title-bar-controls")) return;
    if(w.classList.contains("maxed")) return;
    if(MOBILE&&!w.classList.contains("fixed")) return;
...
function hookIcon(el,ic){
  el.addEventListener("pointerdown",e=>{
    ...
    const up=ev0=>{
      ...
      if(store.data.autoArr){ arrangeIcons(store.data.iconSort); return; }
...
document.addEventListener("contextmenu",e=>{
  ...
  hideMenu();
  if(e.target.closest("#pt-box,.pt-sw")) return;
  if(e.target.closest("#webamp,#webamp-slot")) return;
  if(e.target.closest(".ms-grid")) return;
...
addEventListener("pointerdown",e=>{
  if(e.pointerType!=="touch") return;
  lpConsumed=false;
  if(e.target.closest&&e.target.closest("[data-nolongpress]")) return;
...
function showBalloon(head,text){
  ...
  if(!toastsOn||(sys&&sys.policyOn("nobal"))) return;
...
$("#sv-settings").addEventListener("click",()=>{
  ...
  $("#svo-ok").onclick=()=>{
    ...
    store.save(); miniState=null; miniType=null;
...
function openWinamp(startAt){
  ...
  if(!webamp){
    loadWebamp().then(W=>{
      if(!openApps.has("win-amp")) return;
      try{
        webamp=new W({
...
let localDeaths=0;
let epochLen=150, upT=0, epochStart=0;
...
const FOOD_CHAIN=4;
const TIER_SKINS=["","white","bronze","dinosaur"];
...
function skinCurEl(el,skin){
  ...
  mouse.arenaArrow(skin,u=>{
    ...
    if(old.style.width) img.style.width=old.style.width;
...
function mpUrl(){
  try{
    ...
    if(location.hash.indexOf("#desktop")===0) return null;
...
function mpWelcome(m){
  ...
  setArena(m.epoch.aw,m.epoch.ah);
  if(keepMy){ R.myIn=keepMy.i; R.myOut=keepMy.o; }
  R.pot=m.epoch.pot; R.deploys=m.epoch.deploys; R.deaths=m.epoch.deaths;
  upT=m.epoch.up; epochStart=upT-(m.epoch.eup||0);
...
function mpMsg(m){
  ...
  switch(m.t){
    ...
    case "join": if(MP.on&&m.online){ MP.online=m.online; msn.setHumans(m.online); } break;
...
function enterDesktop(){
  ...
  setTimeout(()=>{
    ...
    if(!first) return;
    if(store.data.tourSeen) showBalloon();
...
if(location.hash.indexOf("#desktop")===0) sessionStorage.setItem("cxp.booted","1");
if(location.hash==="#desktop-start") setTimeout(()=>$("#startmenu").classList.add("open"),400);
if(location.hash==="#desktop-start-mru") setTimeout(()=>{
  openWin("win-help"); openWin("win-mine"); openWin("win-paint");
  minWin("win-help"); minWin("win-mine"); minWin("win-paint");
  $("#startmenu").classList.add("open");
},400);
if(location.hash.indexOf("#desktop-allprog")===0) setTimeout(()=>{
  $("#startmenu").classList.add("open");
  setTimeout(()=>$(".sm-item.allprog").click(),250);
},400);
if(location.hash.indexOf("#desktop-help")===0) setTimeout(()=>{
  openWin("win-help");
  const p=location.hash.replace("#desktop-help","").replace("-","");
  if(p) helpGo(p);
},450);
if(location.hash==="#desktop-logfill") setTimeout(()=>{
  openWin("win-log");
  for(let i=0;i<60;i++) log("mumu > bobo  +0.098   (line "+i+")");
},500);
if(location.hash.indexOf("#desktop-msn")===0) setTimeout(()=>{
...
if(location.hash.indexOf("#desktop-exp")===0) setTimeout(()=>{
  openWin("win-explorer");
...
if(location.hash.indexOf("#desktop-mp")===0) setTimeout(()=>{
...
if(location.hash==="#desktop-ranks") setTimeout(()=>{
  ...
  [[1,"PLANKTON"],[4,"3D-WHITE"],[16,"3D-BRONZE"],[64,"DINOSAUR"]].forEach(([m,nm],i)=>{
    ...
    c.x=320+i*185; c.y=530-i*112;
    c.el.classList.remove("grace");
    c.el.style.transform=`translate(${c.x-8}px,${c.y-4}px)`;
    updateTag(c);
    c.el.querySelector(".mx").textContent="×"+m;
  });
},1400);
if(location.hash==="#desktop-crash") setTimeout(()=>{
  phaseT=Math.min(phaseT,T_SHUT+3);
},2500);
if(location.hash.indexOf("#desktop-cx")===0) setTimeout(()=>{
  openWin("win-cursors");
...
if(location.hash.indexOf("#desktop-dog")===0) setTimeout(()=>{
...
if(location.hash.indexOf("#desktop-sys")===0) setTimeout(()=>{
  $("#balloon").style.display="none";
...
if(location.hash==="#desktop-mouse") setTimeout(()=>mouse.open(),300);
if(location.hash==="#desktop-solitaire") setTimeout(()=>solitaireOpen(),400);
if(location.hash==="#desktop-sound") setTimeout(()=>{
  snd.openMixer(); snd.openRecorder(); snd.openWmp(false);
},400);
if(location.hash==="#desktop-write") setTimeout(()=>{
  write.openNotepad({title:"session.LOG",get:()=>".LOG\nfirst entry",set:null});
  write.openWordpad(null);
  write.openViewer(viewerList(),0);
  write.openClipbook();
},400);
if(location.hash==="#desktop-fileop") setTimeout(()=>{
...
if(location.hash==="#desktop-depth") setTimeout(()=>{
  depth.openDefrag(); depth.openRegedit(); depth.openCalc(); depth.openCharmap();
},400);
if(location.hash.startsWith("#desktop-saver-")) setTimeout(()=>startSaver(location.hash.slice(15)),600);
if(location.hash==="#desktop-mouse-bronze") setTimeout(()=>{
  mouse.applyScheme("bronze");
  setTimeout(()=>{ mouse.open(); $$("#win-mouse .xtab").find(t=>t.dataset.pane==="mo-ptr").click(); },250);
},300);
if(location.hash==="#desktop-vol") setTimeout(()=>{
  $("#balloon").style.display="none"; $("#sndico").click();
},900);
if(location.hash==="#desktop-clock") setTimeout(()=>{
  $("#balloon").style.display="none"; openClockProps();
},900);
if(location.hash==="#desktop-clock-tz") setTimeout(()=>{
  $("#balloon").style.display="none"; openClockProps();
  setTimeout(()=>$$("#win-datetime .xtab").find(t=>t.dataset.pane==="dt-tz").click(),200);
},900);
if(location.hash.indexOf("#desktop-disk")===0) setTimeout(()=>{
...
if(location.hash==="#desktop-binlive") setTimeout(()=>{
  openWin("win-explorer"); explorer.go("Recycle Bin"); explorer.setView("details");
},600);
if(location.hash.indexOf("#desktop-bin")===0&&location.hash!=="#desktop-binlive") setTimeout(()=>{
...
if(location.hash.indexOf("#desktop-paint")===0) setTimeout(()=>{
  openWin("win-paint");
  ...
  tool(7); swatch("#FF0000");
  stroke([[40,40],[70,90],[110,45],[150,95],[190,50]]);
  tool(14); swatch("#0000FF");
  stroke([[210,40],[300,110]]);
  tool(12); swatch("#008000");
  stroke([[40,130],[170,210]]);
  tool(3); swatch("#FFFF00");
  stroke([[100,170]]);
  tool(8); swatch("#FF00FF");
  stroke([[230,150],[250,170],[270,150],[290,180],[250,200]]);
  tool(10); swatch("#000000");
  stroke([[210,230],[330,230]]);
  tool(9);
  if(location.hash==="#desktop-paint-wall"||location.hash==="#desktop-paint-props") setTimeout(()=>{
...
if(location.hash.indexOf("#desktop-mine")===0) setTimeout(()=>{
  ...
  const hidden=[...g.children].filter(e=>e.className==="ms-c");
  hit(hidden[0],2);
  if(hidden[1]){ hit(hidden[1],2); hit(hidden[1],2); }
},500);
if(location.hash==="#desktop-amptest"){
  const dlog=[];
  const D=document.createElement("pre");
